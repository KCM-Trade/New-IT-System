...
export const zhCN = {
  ...
  loginIpsPage: {
    ...
    tradeProfit: {
      ...
      pickRange: "选择日期范围",
      minClients: "最少客户数",
      thresholdClients: "≥ {count} 客户成组",
      excludeSharedExit: "排除共享出口",
      sharedExitThreshold: "共享出口阈值",
      sharedExitOption: "共享出口 ≥ {count} 客户",
      includeSameClient: "含一人多户",
      coverageIncomplete: "日志不完整（当日盈利会偏低）：{list}",
      coverageUnreconciled: "尚未对账：{list}",
      groupsTitle: "账户组（{count}）",
      groupsFootnote:
        "窗口内有 IP 的 {withIp} 单中，{grouped} 单进入了账户组（其余为私有 IP 上的独立账户）",
      colProfit: "组盈亏",
      colProfitTip: "窗口内已实现盈亏合计（USD），按下单时的 IP 归因",
      colClients: "客户数",
      colIbs: "IB 数",
      colIbsTip: "涉及的直属 IB 数（ib_tree level=1）",
      colAccounts: "账户数",
      colTrades: "单数",
      colLots: "手数",
      colSharedIps: "共用 IP",
      colSharedIpsTip:
        "窗口内组成员共用的私有 IP 数；共享出口（客户数 ≥ 阈值）不参与连边",
      colActiveDays: "活跃日",
      colProfitDays: "赚钱日/活跃日",
      colProfitDaysTip: "盈利天数 ÷ 有交易天数",
      colSymbol: "主品种",
      symbolOther: "其他 {pct}%",
      colAvgHold: "平均持仓",
      colAvgHoldTip: "平均每笔持仓时长",
      empty: "本窗口无满足条件的多客户集群",
      emptyHint: "当前条件：≥ {minClients} 客户 · {from} ~ {to}。可调低阈值或扩大窗口。",
      truncated: "共 {total} 组，按盈亏仅显示前 {shown} 组",
      loadFailed: "加载失败：{message}",
      blindSpots:
        "已知盲区：① 每天换手机 IP 且从不共用的团伙抓不到；② 桥接 / KCM\\5LS_* 组约 14% 的下单行没有客户 IP；③ 仅覆盖 IPv4（MT journal 不记录 IPv6）。",
      detailTitle: "组详情",
      detailSummary:
        "组盈亏 {profit} · {accounts} 账户 · {clients} 客户 · 共用 {ips} 个 IP",
      detailNotFound: "当前参数下不存在该组（group_id 随窗口与阈值变化）",
      detailLoadFailed: "详情加载失败：{message}",
      sameClientBadge: "一人多户",
      accountsTitle: "账户明细（{count}）",
      ipsTitle: "使用过的 IP（{count}）",
      clickIpHint: "点击 IP 跳到「手动搜索」预填查询",
      close: "关闭",
      colAccount: "账户",
      colClient: "客户",
      colIb: "直属 IB",
      colCountry: "国家",
      colWindowClients: "窗口客户数",
      colWindowClientsTip:
        "窗口内用过该 IP 的不同 CRM 客户数——越高越像公共出口（≥ 共享出口阈值的 IP 已被剔除、不参与成组）",
      colIpActiveDaysTip: "窗口内该 IP 有下单记录的天数",
      colBridge: "桥接",
      colBridgeTip:
