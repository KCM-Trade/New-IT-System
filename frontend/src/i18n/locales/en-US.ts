...
export const enUS = {
  ...
  loginIpsPage: {
    ...
    // OPT-0063 Phase 3 — trade-IP profit attribution tab (risk-only).
    tradeProfit: {
      window: "Window",
      ...
      minClients: "Min clients",
      thresholdClients: "≥ {count} clients per group",
      excludeSharedExit: "Exclude shared exits",
      sharedExitThreshold: "Shared-exit threshold",
      sharedExitOption: "Shared exit ≥ {count} clients",
      includeSameClient: "Include same-client groups",
      coverageIncomplete: "Incomplete journals (those days read low): {list}",
      coverageUnreconciled: "Not yet reconciled: {list}",
      groupsTitle: "Account groups ({count})",
      groupsFootnote:
        "Of the {withIp} trades with an IP in this window, {grouped} are inside a group (the rest are solo accounts on private IPs)",
      colProfit: "Group P&L",
      colProfitTip:
        "Realised P&L over the window (USD), attributed to the IP each order was placed from",
      colClients: "Clients",
      colIbs: "IBs",
      colIbsTip: "Distinct direct IBs touched (ib_tree level 1)",
      colAccounts: "Accounts",
      colTrades: "Trades",
      colLots: "Lots",
      colSharedIps: "Shared IPs",
      colSharedIpsTip:
        "Private IPs shared inside the group; shared exits (clients ≥ threshold) never form edges",
      colActiveDays: "Active days",
      colProfitDays: "Profitable/active",
      colProfitDaysTip: "Days in profit ÷ days traded",
      colSymbol: "Top symbol",
      symbolOther: "other {pct}%",
      colAvgHold: "Avg hold",
      colAvgHoldTip: "Mean holding time per trade",
      empty: "No multi-client clusters match in this window",
      emptyHint:
        "Current: ≥ {minClients} clients · {from} ~ {to}. Lower the thresholds or widen the window.",
      truncated: "{total} groups in this window — showing the top {shown} by P&L",
      loadFailed: "Load failed: {message}",
      blindSpots:
        "Known blind spots: ① crews that rotate mobile IPs daily and never share one are invisible; ② bridge / KCM\\5LS_* groups have no client IP on ~14% of order lines; ③ IPv4 only (MT journals do not record IPv6).",
      detailTitle: "Group detail",
      detailSummary:
        "P&L {profit} · {accounts} accounts · {clients} clients · {ips} shared IPs",
      detailNotFound:
        "This group does not exist under the current parameters (group_id encodes the window and thresholds)",
      detailLoadFailed: "Failed to load the group: {message}",
      sameClientBadge: "Same client",
      accountsTitle: "Accounts ({count})",
      ipsTitle: "IPs used ({count})",
      clickIpHint: "Click an IP to jump to the Search tab with it pre-filled",
      close: "Close",
      colAccount: "Account",
      colClient: "Client",
      colIb: "Direct IB",
      colCountry: "Country",
      colWindowClients: "Window clients",
      colWindowClientsTip:
        "Distinct CRM clients seen on this IP in the window — higher means more like a public exit (IPs at or above the shared-exit threshold are excluded from grouping)",
      colIpActiveDaysTip: "Days in the window this IP placed orders",
      colBridge: "Bridge",
      colBridgeTip:
